...
class OhioSettings {
	...
	/**
	 * Get color scheme of the current page
	 *
	 * @return string (light|dark|auto)
	 */
	static public function get_color_mode() {
		// We need to use these because of the backward compatibility
		$DARK_MODE = '1';
		$AUTO_MODE = '2';

		$page_color_mode = OhioOptions::get( 'page_dark_mode', 'light' );
		if ( $page_color_mode == $AUTO_MODE ) return 'auto';

		$page_dark_mode = $page_color_mode == $DARK_MODE;
		...
		if ( ( !$save_color_mode || !$cookie_set ) && $page_dark_mode ) return 'dark';
		if ( $save_color_mode && ( $page_switcher || ( !$page_switcher && $page_dark_mode ) ) && $cookie_set && $cookie_dark_mode ) return 'dark';
		return 'light';
	}
}