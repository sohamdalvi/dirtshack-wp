...
<div class="yith-wcwl-share">
    ...
    <div class="social-networks -small">
        <?php if ( $share_facebook_enabled ) : ?>
            <a target="_blank" class="network -unlink facebook" href="https://www.facebook.com/sharer.php?u=<?php echo urlencode($share_link_url)?>&p[title]=<?php echo $share_link_title ?>" title="<?php _e( 'Facebook', 'ohio' ) ?>">
                <?php echo $share_facebook_icon ? '<i class="icon fa-brands fa-facebook-f"></i>' : __( 'Facebook', 'ohio' ) ?>
            </a>
        <?php endif; ?>

        <?php if ( $share_twitter_enabled ) : ?>
            <a target="_blank" class="network -unlink twitter" href="https://x.com/share?url=<?php echo urlencode( $share_link_url ) ?>&amp;text=<?php echo $share_twitter_summary ?>" title="<?php _e( 'X', 'ohio' ) ?>">
                <?php echo $share_twitter_icon ? '<i class="icon fa-brands fa-x-twitter"></i>' : __( 'X', 'ohio' ) ?>
            </a>
        <?php endif; ?>

        <?php if ( $share_pinterest_enabled ) : ?>
            <a target="_blank" class="network -unlink pinterest" href="http://pinterest.com/pin/create/button/?url=<?php echo urlencode( $share_link_url ) ?>&amp;description=<?php echo $share_summary ?>&amp;media=<?php echo $share_image_url ?>" title="<?php _e( 'Pinterest', 'ohio' ) ?>" onclick="window.open(this.href); return false;">
                <?php echo $share_pinterest_icon ? '<i class="icon fa-brands fa-pinterest"></i>' : __( 'Pinterest', 'ohio' ) ?>
            </a>
        <?php endif; ?>

        <?php if ( $share_email_enabled ) : ?>
            <a class="network -unlink email" href="mailto:?subject=<?php echo urlencode( apply_filters( 'yith_wcwl_email_share_subject', $share_link_title ) )?>&amp;body=<?php echo apply_filters( 'yith_wcwl_email_share_body', urlencode( $share_link_url ) ) ?>&amp;title=<?php echo $share_link_title ?>" title="<?php _e( 'Email', 'ohio' ) ?>">
                <?php echo $share_email_icon ? '<i class="icon fa-regular fa-envelope"></i>' : __( 'Email', 'ohio' ) ?>
            </a>
        <?php endif; ?>

        <?php if ( $share_whatsapp_enabled ) : ?>
            <a class="network -unlink whatsapp" href="<?php echo $share_whatsapp_url ?>" data-action="share/whatsapp/share" target="_blank" title="<?php _e( 'WhatsApp', 'ohio' ) ?>">
                <?php echo $share_whatsapp_icon ? '<i class="icon fa-brands fa-whatsapp"></i>' : __( 'Whatsapp', 'ohio' ) ?>
            </a>
        <?php endif;
        ?>
    </div>
