...
<?php
	...
	if ( OhioOptions::get( 'portfolio_randomized_order', false ) ) {
		shuffle($projects_array);
	}
	...
	if ( !$wrap_container ) {
		$page_container_class .= ' -full-w -reset-padding';
	}
	...
	$project_layout = OhioOptions::get( 'portfolio_item_layout_type', 'grid_1' );
			...
			<main id="main" class="site-main">
				...
				<?php if ( !$is_slider ) : ?>
					<?php
						...
						if ( $project_layout == 'grid_8' ) {
							$grid_attr .= "data-interactive-links-grid=true";
						}
					?>

					<div class="<?php if ( $project_layout == 'grid_8' || $project_layout == 'grid_12' ) { echo 'portfolio-links' . esc_attr( $fullscreen_class ); } ?> <?php echo esc_attr( $project_layout . $wrapper_classes ); ?>" data-ohio-portfolio-grid="true" <?php echo esc_attr( $grid_attr ); ?>>

						<?php if ( $project_layout == 'grid_8' || $project_layout == 'grid_12' ) : ?>
							<div class="project-content">
								<div class="portfolio-grid-images"<?php if ( $project_layout == 'grid_8' ) : ?>data-vc-full-width="true" data-vc-stretch-content="true"<?php endif; ?>></div>
							</div>
						<?php endif; ?>
								...
								<?php if ( is_array( $cat_ids ) && $cat_ids ) : ?>
									...
									<div class="portfolio-filter<?php echo esc_attr( $filter_classes ); ?>"
										...
										<ul class="-unlist">
											...
											<li>
												<a class="active<?php if ( $filter_layout == 'button' ) { echo esc_attr( ' tag'); } ?>" href="#all" data-isotope-filter="*" data-category-count="<?php echo str_pad( (string)$published_projects, 2, '0', STR_PAD_LEFT ); ?>">
											...
											<?php foreach ( $cat_ids as $cat_obj ) : ?>
												...
												?>
												<li>
													<a class="<?php if ( $filter_layout == 'button' ) { echo esc_attr( ' tag'); } ?>" href="#<?php echo esc_attr( $cat_obj->slug ); ?>" data-isotope-filter=".ohio-filter-project-<?php echo hash( 'md4', $cat_obj->slug, false ); ?>" data-category-count="<?php echo esc_attr( $cat_obj->count ); ?>">
							...
							<?php if ( $project_layout == 'grid_12' ) : ?>

								<div class="portfolio-grid-holder">

							<?php endif; ?>

							<div class="portfolio-grid archive-holder vc_row<?php echo esc_attr( $grid_offset_class . $reversed_classes ); ?>" data-lazy-container="projects" data-isotope-grid="true" data-projects-per-page="<?php echo esc_attr( $projects_per_page); ?>">
							...
							foreach ( $projects_array as $_project_index => $_project_object ) {
								...
								if ( $project_layout == 'grid_1' || $project_layout == 'grid_2' || $project_layout == 'grid_11' || $project_layout == 'grid_12' || $project_layout == 'grid_13' ) {
								...
								// Grid calculating
								$col_class = '';
								if ( $project_layout != 'grid_8' && $project_layout != 'grid_12' ) {
									$col_class = $columns_class;

									if ( $ohio_project['grid_style'] == '2col' && $project_layout != 'grid_13' ) {
				...
				<?php if ( $is_slider ) : ?>
					...
					<div class="portfolio-onepage-slider -slider-fs clb-slider-scroll-bar <?php echo esc_attr( $project_layout ); ?>"
						...
						data-slider-columns="<?php if ( $project_layout === 'grid_6' ) echo esc_attr( $slider_columns ); ?>"
