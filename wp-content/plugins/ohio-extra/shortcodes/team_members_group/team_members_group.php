...
function ohio_team_members_group_func( $atts, $content = '' ) {
	...
	$content_bg = isset( $content_bg ) ? OhioExtraFilter::string( $content_bg ) : false;
	...
	*/
	...
	$content_bg = OhioExtraParser::VC_color_to_CSS( $content_bg, '{{color}}' );
	if ( $content_bg ) {
		$_style_block .= '#' . $wrapper_id . '.team-group .item-holder{';
		$_style_block .= 'background-color:' . $content_bg . ';';
		$_style_block .= '}';
	}
