...
function ohio_badge_func( $atts ) {
	...
	$text_before = isset( $text_before ) ? OhioExtraFilter::string( $text_before, 'string', '' ) : '';
	...
	$link = isset( $link ) ? OhioExtraFilter::boolean( $link ) : false;
	...
	$text_before_color = isset( $text_before_color ) ? OhioExtraFilter::string( $text_before_color, 'string', false ) : false;
	...
	*/
	...
	$badge_text_before_typo = OhioExtraParser::VC_typo_to_CSS( $text_before_typo );
	OhioExtraParser::VC_typo_custom_font( $text_before_typo );

	if ( $badge_text_before_typo ) {
		$_selector = '#' . $wrapper_id . ' .badge-inner{';
		$_block_typo = $badge_text_before_typo;
		if ( !empty( $_block_typo['desktop'] ) ) {
			$_style_block .= $_selector . $_block_typo['desktop'] . '}';
		}
		if ( !empty( $_block_typo['tablet'] ) ) {
	...
	$badge_text_before_color = OhioExtraParser::VC_color_to_CSS( $text_before_color, '{{color}}' );
	if ( $badge_text_before_color ) {
		$_style_block .= '#' . $wrapper_id . ' .badge-inner{';
		$_style_block .= 'background-color:' . $badge_text_before_color . ';';
		$_style_block .= '}';
	}
