...
<div class="ohio-widget portfolio-projects<?php echo esc_attr( $wrapper_classes ); ?> <?php if ( $is_slider ) echo 'vc_row'; ?>" id="<?php echo esc_attr( $wrapper_id ); ?>" <?php echo esc_attr( $animation_attrs); ?> data-ohio-portfolio-grid="true">
	...
	<?php if ( $show_projects_filter && !$is_slider ): ?>
		...
		>
			<ul class="-unlist">
				...
				<li>
                    <a class="active<?php if ( $filter_layout == 'button' ) { echo esc_attr( ' tag'); } ?>" href="#all" data-isotope-filter="*" data-category-count="<?php echo str_pad( (string)$all_projects_count, 2, '0', STR_PAD_LEFT ); ?>">
	...
	<?php if ( !$is_slider ): ?>
		...
		<div class="vc_row portfolio-grid<?php if ( preg_replace("/[^0-9.]/", '', (string) $grid_items_gap ) == '0' ) { echo esc_html( ' -nospace' ); } ?>" data-isotope-grid="true" data-lazy-container="projects" data-projects-per-page="<?php echo esc_attr( $projects_in_block); ?>">
			...
			<?php
				...
				if ( $use_pagination ) {
					...
					$projects_data = array_slice( $projects_data, $_post_start, $_post_end );
				}

				$_prev_item_featured_image = '';
			?>

			<?php for ( $_post_i = 0; $_post_i < $_post_end - $_post_start; $_post_i++ ): ?>
			<?php
				...
				$ohio_project['show_featured_video'] = $show_featured_video;
				...
				if ( $_post_i == 0) {
					$_last_project = OhioObjectParser::parse_to_project_object( $projects_data[$_post_end - $_post_start - 1] );
	...
	<?php else : ?>
		<div class="portfolio-onepage-slider -slider-fs clb-slider-scroll-bar <?php echo esc_attr( $card_layout . $pagination_type_class ); ?>" <?php echo esc_attr( $slider_attrs); ?>>
