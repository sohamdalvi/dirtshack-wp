...
function ohio_pricing_table_func( $atts ) {
	...
	$table_button_position = isset( $table_button_position ) ? OhioExtraFilter::string( $table_button_position, 'string', 'default' ) : 'default';
	...
	$old_price_typo = isset( $old_price_typo ) ? OhioExtraFilter::string( $old_price_typo ) : false;
	...
	*/
	...
	$table_old_price_typo = OhioExtraParser::VC_typo_to_CSS( $old_price_typo );
	OhioExtraParser::VC_typo_custom_font( $old_price_typo );

	if ( $table_old_price_typo ) {
		$_selector = '#' . $wrapper_id . ' .old-price{';
		$_block_typo = $table_old_price_typo;
		if ( !empty( $_block_typo['desktop'] ) ) {
			$_style_block .= $_selector . $_block_typo['desktop'] . '}';
		}
		if ( !empty( $_block_typo['tablet'] ) ) {
	...
	if ( $features_tooltip_bg_color ) {
		$_style_block .= '#' . $wrapper_id . ' .pricing-table-features .has-tooltip::before{';
		$_style_block .= 'background-color:' . $features_tooltip_bg_color . ';';
		$_style_block .= '}';
		$_style_block .= '#' . $wrapper_id . ' .pricing-table-features .has-tooltip::after{';
	...
	if ( $table_tooltip_typo ) {
		$_selector = '#' . $wrapper_id . ' .pricing-table-features .has-tooltip::before{';
	...
	$border_color = OhioExtraParser::VC_color_to_CSS( $border_color, 'border-color:{{color}};' );
	if ( $border_color ) {
		$_style_block .= '#' . $wrapper_id . '.pricing-table{';
		$_style_block .= $border_color;
		$_style_block .= '}';
	}
