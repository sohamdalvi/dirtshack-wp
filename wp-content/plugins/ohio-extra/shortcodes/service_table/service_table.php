...
function ohio_service_table_func( $atts ) {
	...
	$icon_border_width = isset( $icon_border_width ) ? OhioExtraFilter::string( $icon_border_width, 'string', '') : '';
	$icon_corners = isset( $icon_corners ) ? OhioExtraFilter::string( $icon_corners, 'string', '') : '';
	...
	$features_tooltip_bg_color = isset( $features_tooltip_bg_color ) ? OhioExtraFilter::string( $features_tooltip_bg_color ) : false;
	$features_tooltip_typo = isset( $features_tooltip_typo ) ? OhioExtraFilter::string( $features_tooltip_typo ) : false;
	...
	$border_color = isset( $border_color ) ? OhioExtraFilter::string( $border_color, 'string', false ) : false;
	...
	if ( $features_value ) {
		foreach ($features_value as $feature_key => $feature_value) {
			...
			if ( isset( $feature_value->feature_tooltip ) ) {
				$features_value[$feature_key]->feature_tooltip = OhioExtraFilter::string( $feature_value->feature_tooltip );
			} else {
				$features_value[$feature_key]->feature_tooltip = false;
			}
		}
	}
	...
	*/
	...
	$features_tooltip_bg_color = OhioExtraParser::VC_color_to_CSS( $features_tooltip_bg_color, '{{color}}' );
	if ( $features_tooltip_bg_color ) {
		$_style_block .= '#' . $wrapper_id . ' .service-table-features .has-tooltip::before{';
		$_style_block .= 'background-color:' . $features_tooltip_bg_color . ';';
		$_style_block .= '}';
		$_style_block .= '#' . $wrapper_id . ' .service-table-features .has-tooltip::after{';
		$_style_block .= 'border-top-color:' . $features_tooltip_bg_color . ';';
		$_style_block .= '}';
	}

	$table_tooltip_typo = OhioExtraParser::VC_typo_to_CSS( $features_tooltip_typo );
	OhioExtraParser::VC_typo_custom_font( $features_tooltip_typo );

	if ( $table_tooltip_typo ) {
		$_selector = '#' . $wrapper_id . ' .service-table-features .has-tooltip::before{';
		$_block_typo = $table_tooltip_typo;
		if ( !empty( $_block_typo['desktop'] ) ) {
			$_style_block .= $_selector . $_block_typo['desktop'] . '}';
		}
		if ( !empty( $_block_typo['tablet'] ) ) {
	...
	if ( isset( $border_width ) && $border_width != '' ) {
		$_style_block .= '#' . $wrapper_id . '.service-table{';
		$_style_block .= 'border:' . $border_width . 'px solid transparent;';
		$_style_block .= '}';
	...
	if ( isset( $icon_border_width ) && $icon_border_width != '' ) {
		if ( $icon_layout === 'border' ) {
			$_style_block .= '#' . $wrapper_id . ' .icon-group.-outlined{';
			$_style_block .= 'border-width:' . $icon_border_width . 'px;';
			$_style_block .= '}';
		}
		if ( $icon_layout === 'fill' ) {
			$_style_block .= '#' . $wrapper_id . ' .icon-group.-contained{';
			$_style_block .= 'border:' . $icon_border_width . 'px solid;';
			$_style_block .= '}';
		}
	}
	...
	if ( isset( $icon_corners ) && $icon_corners != '' ) {
		$_style_block .= '#' . $wrapper_id . ' .icon-group.-outlined,';
		$_style_block .= '#' . $wrapper_id . ' .icon-group.-contained{';
		$_style_block .= 'border-radius:' . $icon_corners . 'px;';
		$_style_block .= '}';
	}
