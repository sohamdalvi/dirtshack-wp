...
function ohio_clients_logo_func( $atts ) {
	...
	$set_logo_width = isset( $set_logo_width ) ? OhioExtraFilter::boolean( $set_logo_width ) : false;

	$clients_logo_height = isset( $clients_logo_height ) ? OhioExtraFilter::string( $clients_logo_height, 'string', '54') : '54';
	$set_logo_height = isset( $set_logo_height ) ? OhioExtraFilter::boolean( $set_logo_height ) : true;
	...
	$image_logo_inverse = ( isset( $image_logo_inverse ) ) ? OhioExtraFilter::string( $image_logo_inverse ) : false;
	$image_logo_inverse_atts = OhioExtraParser::generateImageAttsById( OhioExtraFilter::string( $image_logo_inverse ), $title );
	$image_logo_inverse = ( !empty( $image_logo_inverse ) ) ? OhioExtraFilter::string( wp_get_attachment_url( $image_logo_inverse ), 'attr' ) : false;
	...
	*/
	...
	if ( $set_logo_width && $clients_logo_width ) {
		$_style_block .= '#' . $wrapper_id . '.logo img{';
		$_style_block .= 'width:' . $clients_logo_width . 'px';
		$_style_block .= '}';
	}

	if ( $set_logo_height && $clients_logo_height ) {
		$_style_block .= '#' . $wrapper_id . '.logo img{';
		$_style_block .= 'height:' . $clients_logo_height . 'px';
		$_style_block .= '}';
	}
