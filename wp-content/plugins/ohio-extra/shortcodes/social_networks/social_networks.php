...
function ohio_social_networks_func( $atts ) {
	...
	$border_hover_color = isset( $border_hover_color ) ? OhioExtraFilter::string( $border_hover_color, 'string', false ) : false;
	...
	$threads_link_custom = isset( $threads_link_custom ) ?  OhioExtraFilter::string( $threads_link_custom ) : false;
	...
	switch ( $block_alignment ) {
		case 'left':
			$content_classes .= ' -flex-just-start';
			break;
		case 'center':
		default:
			$content_classes .= ' -flex-just-center';
			break;
		case 'right':
			$content_classes .= ' -flex-just-end';
			break;
	}
	...
	if ( $type_links == 'share' ) {
		...
		$twitter_link_custom = ( $twitter ) ? 'https://x.com/intent/tweet?text=' . get_permalink() : '';
	...
	*/
	...
	if ( $hover_color ) {
		if ( $icon_layout ==  'flat' ) {
			$_style_block .= '#' . $wrapper_id . '.social-networks.-flat .network:hover{';
			$_style_block .= 'background-color:' . $hover_color . ';';
		} elseif ( $icon_layout ==  'text' || $icon_layout ==  'inline' ) {
			$_style_block .= '#' . $wrapper_id . '.social-networks.-text:not(.-boxed) .network:hover{';
			...
			$_style_block .= '#' . $wrapper_id . '.social-networks.-boxed .network:hover{';
			$_style_block .= 'background-color:' . $hover_color . ';';
		}
		$_style_block .= '}';
	}

	// Filled layout only, other layouts are handled above
	$border_color = OhioExtraParser::VC_color_to_CSS( $border_color, '{{color}}' );
	if ( $border_color ) {

		if ( $icon_layout == 'boxed' ) {
			$_style_block .= '#' . $wrapper_id . '.social-networks.-boxed{';
			$_style_block .= 'border-color:' . $border_color . ';';
			$_style_block .= '}';
		} else {
			$_style_block .= '#' . $wrapper_id . '.social-networks.-contained .network{';
			$_style_block .= 'border-color:' . $border_color . ';';
			$_style_block .= '}';
		}
	}
	...
	if ( $border_hover_color ) {
		$_style_block .= '#' . $wrapper_id . '.social-networks .network:hover{';
		$_style_block .= 'border-color:' . $border_hover_color . ';';
		$_style_block .= '}';
	}
	...
	if ( $icon_color ) {
		if ( $icon_layout ==  'boxed' ) {
			$_style_block .= '#' . $wrapper_id . '.social-networks.-boxed .network i + span{';
			$_style_block .= 'color:' . $icon_color . ';';
		} else {
			$_style_block .= '#' . $wrapper_id . ' .network{';
			$_style_block .= 'color:' . $icon_color . ';';
		}
		$_style_block .= '}';
	}
	...
	if ( $icon_hover_color ) {
		if ( $icon_layout ==  'boxed' ) {
			$_style_block .= '#' . $wrapper_id . '.social-networks.-boxed .network:hover i{';
			$_style_block .= 'color:' . $icon_hover_color . ';';
		} else {
			$_style_block .= '#' . $wrapper_id . ' .network:hover{';
			$_style_block .= 'color:' . $icon_hover_color . ';';
		}
		$_style_block .= '}';
	}
	...
	if ( isset( $border_radius ) && $border_radius != '' ) {
		if ( $icon_layout == 'boxed' ) {
			...
			$_style_block .= '#' . $wrapper_id . '.social-networks:not(.-boxed):not(.-text) .network{';
			$_style_block .= 'border-radius:' . $border_radius . 'px;';
			$_style_block .= '}';
		}
	}
	...
	if ( isset( $border_width ) && $border_width != '' ) {
		if ( $icon_layout == 'outline' ) {
			...
			$_style_block .= 'border-style: solid;';
			$_style_block .= '}';
		} elseif ( $icon_layout == 'boxed' ) {
			$_style_block .= '#' . $wrapper_id . '.social-networks.-boxed{';
			$_style_block .= 'border-width:' . $border_width . 'px;';
			$_style_block .= 'border-style: solid;';
			$_style_block .= '}';
		} else {
			$_style_block .= '#' . $wrapper_id . '.social-networks:not(.-outlined):not(.-contained):not(.-boxed) .network:hover{';
			$_style_block .= 'border-width:' . $border_width . 'px;';
			$_style_block .= 'border-style: solid;';
			$_style_block .= '}';
		}
