...
function ohio_recent_posts_func( $atts ) {
	...
	$shortcode = 'ohio_recent_posts';
	$sc64 = OhioExtraParser::encodeShortcodeParams( $atts );

	// Default values, parsing and filtering
	$posts = isset( $posts ) ? OhioExtraFilter::string( $posts, 'attr', 'all' ) : 'all';
	$post_category = isset( $post_category ) ? OhioExtraFilter::string( $post_category, 'string', 'all' ) : 'all';
	$order = isset( $order ) ? OhioExtraFilter::string( $order, 'string', 'date' ) : 'date';
	$orderby = isset( $orderby ) ? OhioExtraFilter::string( $orderby, 'string', 'DESC' ) : 'DESC';
	...
	$show_author = isset( $show_author ) ? OhioExtraFilter::boolean( $show_author ) : true;
	$show_categories = isset( $show_categories ) ? OhioExtraFilter::boolean( $show_categories ) : true;
	$show_reading_time = isset( $show_reading_time ) ? OhioExtraFilter::boolean( $show_reading_time ) : true;
	$show_date = isset( $show_date ) ? OhioExtraFilter::boolean( $show_date ) : true;
	$card_gap = isset( $card_gap ) ? OhioExtraFilter::string( $card_gap, 'string', '' ) : '';
	...
	if ( preg_replace("/[^0-9.]/", '', (string) $card_gap ) == '0' ) {
		$wrapper_classes .= ' -nospace';
	}
	...
	*/
	...
	$style_class = [];
	if ( in_array( $pagination_style, [ 'default', 'outlined', 'flat' ], true ) ) {
		$style_class[] = '-' . $pagination_style;
	}
	...
	*/
	...
	if ( $card_excerpt_typo ) {
		...
		$_selector = '#' . $wrapper_id . ' .blog-item .card-details-item > p{';
	...
	if ( $card_layout != 'blog_grid_6' || $card_boxed == true ) {
		if ( $card_gap ) {
			...
			$_style_block .= '@media screen and (min-width: 769px){';
			$_style_block .= '#' . $wrapper_id . '.vc_row:not(.-nospace).blog-posts{';
	...
	if ( $pagination_color ) {
		$_style_block .= '#' . $wrapper_id . ' .pagination,';
		$_style_block .= '#' . $wrapper_id . ' .lazy-load{';
		$_style_block .= '--clb-color-paginator-button:' . $pagination_color . ';';
		$_style_block .= '}';
	}
	...
	if ( $pagination_active_color ) {
		$_style_block .= '#' . $wrapper_id . ' .pagination,';
		$_style_block .= '#' . $wrapper_id . ' .lazy-load{';
		$_style_block .= '--clb-color-paginator-button-hover:' . $pagination_active_color . ';';
		$_style_block .= '}';
	}
