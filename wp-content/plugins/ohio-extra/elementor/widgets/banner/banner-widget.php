    ...
    protected function register_controls()
    {
        ...
        $this->add_control(
            ...
            [
                ...
                'type' =>  \Elementor\Controls_Manager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} .banner:not(.-with-overlay):not(.-image-only):not(.-with-overlay-image) .overlay-details' => 'background: linear-gradient(rgba(0, 0, 0, 0), {{VALUE}})',
                    '{{WRAPPER}} .banner.-with-overlay-image:hover .overlay-details' => 'background-color: {{VALUE}}',
                    '{{WRAPPER}} .banner:not(.-with-overlay-image) .overlay-details' => 'background-color: {{VALUE}}'
                ],
                'separator' => 'before'
            ]
        );
    ...
    protected function render()
    {
        ...
        switch ( $settings['block_type_layout'] ) {
            case 'inner':
                ...
                break;
            case 'overlay_image':
                $this->addWrapperClass( '-with-overlay-image' );
                break;
            case 'inner_hover':
        ...
        if ( $settings['button_animation'] ) {
            $this->addWrapperClass( '-with-icon-button-animation' );
        }
