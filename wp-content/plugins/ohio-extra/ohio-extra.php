...
/*
	...
	Copyright 2025 Colabrio (email: support@clbthemes.com)
...
if ( in_array( $ohio_extra_get_theme->get( 'TextDomain' ), array( 'ohio', 'ohio-child' ) ) ) {
	...
	function ohio_share_woo_func() {
		...
		$twitter_link = 'https://x.com/intent/tweet?text=' . urlencode( $post->post_title ) . ',+' . rawurlencode( get_permalink() );
		...
		<div class="share-bar -vertical">
			...
			<?php
				foreach ( $social_networks as $link ) {
					switch ( $link ) {
						case 'facebook':
							echo '<a class="-unlink" href="' . $facebook_link . '" target="' . $target_blank . '" aria-label="' . esc_attr( 'Facebook', 'ohio-extra' ) . '"><i class="network fa-brands fa-facebook-f"></i></a>';
							break;
						case 'twitter':
							echo '<a class="-unlink" href="' . $twitter_link . '" target="' . $target_blank . '" aria-label="' . esc_attr( 'X', 'ohio-extra' ) . '"><i class="network fa-brands fa-x-twitter"></i></a>';
							break;
						case 'linkedin':
							echo '<a class="-unlink" href="' . $linkedin_link . '" target="' . $target_blank . '" aria-label="' . esc_attr( 'Linkedin', 'ohio-extra' ) . '"><i class="network fa-brands fa-linkedin"></i></a>';
							break;
						case 'pinterest':
							echo '<a class="-unlink" href="' . $pinterest_link . '" target="' . $target_blank . '" aria-label="' . esc_attr( 'Pinterest', 'ohio-extra' ) . '"><i class="network fa-brands fa-pinterest"></i></span></a>';
							break;
					}
				}
			?>
		...
		<div class="share-bar -vertical" data-blog-share="true">
			...
			<?php
				foreach ( $social_networks as $link ) {
					switch ( $link ) {
						case 'facebook':
							echo '<a href="' . $facebook_link . '" target="' . $target_blank . '" class="facebook network -unlink" aria-label="' . esc_attr( 'Facebook', 'ohio-extra' ) . '"><i class="fa-brands fa-facebook-f"></i></a>';
							break;
						case 'twitter':
							echo '<a href="' . $twitter_link . '" target="' . $target_blank . '" class="twitter network -unlink" aria-label="' . esc_attr( 'X', 'ohio-extra' ) . '"><i class="fa-brands fa-x-twitter"></i></a>';
							break;
						case 'linkedin':
							echo '<a href="' . $linkedin_link . '" target="' . $target_blank . '" class="linkedin network -unlink" aria-label="' . esc_attr( 'Linkedin', 'ohio-extra' ) . '"><i class="fa-brands fa-linkedin"></i></a>';
							break;
						case 'pinterest':
							echo '<a href="' . $pinterest_link . '" target="' . $target_blank . '" class="pinterest network -unlink" aria-label="' . esc_attr( 'Pinterest', 'ohio-extra' ) . '"><i class="fa-brands fa-pinterest"></i></a>';
							break;
					}
				}
			?>
	...
	function ohio_current_year_func() {
		$year = date_i18n( 'Y' );
		return $year;
	}
	add_shortcode( 'ohio_current_year', 'ohio_current_year_func' );

	function ohio_site_name_func() {
		$site_name = get_bloginfo( 'name' );
		return $site_name;
	}
	add_shortcode( 'ohio_site_name', 'ohio_site_name_func' );
	...
	function ohio_extra_vc_init_plugin() {
        ...
        require_once $types_path . 'posts.php'; // Dropdown with post categories
	...
	function ohio_admin_bar_link( $wp_admin_bar ) {
		...
		array_push( $args, array(
			'id'        => 'appearance',
			'title'     => esc_html__( 'Appearance', 'ohio-extra' ),
			'href'      => admin_url( 'admin.php?page=ohio_hub_settings&options_page=theme-appearance' ),
			'parent'    => 'theme-settings',
		));
		array_push( $args, array(
			'id'		=>	'typography',
		...
		array_push( $args, array(
			'id'		=>	'post-settings',
			'title'		=>	esc_html__( 'Single Post', 'ohio-extra' ),
		...
		array_push( $args, array(
			'id'		=>	'project-settings',
			'title'		=>	esc_html__( 'Single Project', 'ohio-extra' ),
			'href'		=>	admin_url( 'admin.php?page=ohio_hub_settings&options_page=theme-general-project' ),
			'parent'	=>	'theme-settings',
		));
		array_push( $args, array(
			'id'		=>	'shop-settings',
		...
		array_push( $args, array(
			'id'		=>	'product-settings',
			'title'		=>	esc_html__( 'Single Product', 'ohio-extra' ),
			'href'		=>	admin_url( 'admin.php?page=ohio_hub_settings&options_page=theme-general-product' ),
			'parent'	=>	'theme-settings',
		));
		array_push( $args, array(
			'id'		=>	'custom-settings',
			'title'		=>	esc_html__( 'Custom CSS & JS', 'ohio' ),
			'href'		=>	admin_url( 'admin.php?page=ohio_hub_settings&options_page=theme-general-custom' ),
			'parent'	=>	'theme-settings',
		));
		array_push( $args, array(
			'id'		=>	'maintenance-settings',
			'title'		=>	esc_html__( 'Maintenance Mode', 'ohio-extra' ),
			'href'		=>	admin_url( 'admin.php?page=ohio_hub_settings&options_page=theme-general-maintenance' ),
			'parent'	=>	'theme-settings',
		));
		array_push( $args, array(
			'id'		=>	'other-settings',
			'title'		=>	esc_html__( 'Performance', 'ohio-extra' ),
			'href'		=>	admin_url( 'admin.php?page=ohio_hub_settings&options_page=theme-general-performance' ),
			'parent'	=>	'theme-settings',
		));
		array_push( $args, array(
			'id'		=>	'other-settings',
		...
		array_push( $args, array(
			'id'		=>	'backup-settings',
			'title'		=>	esc_html__( 'Backup & Reset', 'ohio-extra' ),
