...
if ( ! empty( $parallax_image ) ) {
	if ( $has_video_bg ) {
		...
		$parallax_image_id = preg_replace( '/[^\d]/', '', (string) $parallax_image );
...
if ( $use_through_lines ) {
	$ohio_lines_class = 'row-bg-lines -unlist';

	$output .= '<ul class="' . $ohio_lines_class . '">';
	for( $i = 0; $i < 4; $i++ ){
		$output .= '<li></li>';
	}
	$output .= '</ul>';
